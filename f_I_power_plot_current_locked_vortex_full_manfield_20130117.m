...
threshold = 2.5;           % Threshold for peak detection. NOTE this is not
...
REGROUP = 2;             % Set to 1 to sort based on frequency similarity
                         ...
                         % the "_org2" files.
LowFreq = 150e-3;             % Minimum frequency to start scan in GHz
WindowFreq = [150e-3 200e-3];     % Window to estimate the measurement noise in GHz.
...
for i=1:length(current_log)
    ...
    sampleID=sampleID{1};
    [temp1 rff]=textread(fid,'%s %f',1,'headerlines',5);
    [temp1 rfp]=textread(fid,'%s %f',1,'headerlines',6);
    [temp1 current(i)]=textread(fid,'%s %f',1,'headerlines',7);
    [temp1 temp2 temp3 field(i)]=textread(fid,'%s %s %s %f',1,'headerlines',8);
...
export_vector = [r_count field current rr];
for ii = 1 : GROUP
...
figure; clf;
plot(abs(rff_log)*1e-6, peak_freq_org3/1e9, '.');

figure; clf;
plot(abs(rff_log)*1e-6, int_power_org3/1e-12, '.');

figure; clf;
plot(abs(rff_log)*1e-6, linewidth_org3/1e6, '.');