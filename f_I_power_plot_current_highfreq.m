...
amp_gain = 22;           % Gain of the amplifier used during measurements
...
REGROUP = 2;             % Set to 1 to sort based on frequency similarity
                         ...
                         % the "_org2" files.
LowFreq = 19;             % Minimum frequency to start scan in GHz
WindowFreq = [21.8 21.9];     % Window to estimate the measurement noise in GHz.
...
figure; clf;
plot(abs(current)*1e3, int_power_org2/1e-12, '.');

figure; clf;
plot(abs(current)*1e3, linewidth_org2/1e6, '.');