...
threshold = 2.3;           % Threshold for peak detection. NOTE this is not
                         ...
                         % of 2 is suggested.
N_attempt = 5;           % Allow several attempts to find the best fit. For
...
WindowFreq = [100e-3 120e-3];     % Window to estimate the measurement noise in GHz.
...
% Otherwise, runs the main program
    ...
    [RFF_log dev_log RFP_log current_log spectrum_path]=textread(log_path,...
    '%f %f %f %f %s',-1,'headerlines',7,'delimiter','\t'); % 7 is standard
end
...
for c=2%:length(seqfig1_unique)
% for j=length(seqfig1_unique)-1
for d=1:length(seqdev_unique)
    logindex=find((abs(seqfig1-seqfig1_unique(c))<0.001) & (abs(seqdev-seqdev_unique(d))<0.001) & (abs(seqfig2-seqfig2_unique(m))<0.001));
...
for z=1:length(logindex)
    ...
    [temp1 deviation]=textread(fid,'%s %f',1,'headerlines',6);
    [temp1 rfp]=textread(fid,'%s %f',1,'headerlines',7);
    [temp1 current(z)]=textread(fid,'%s %f',1,'headerlines',8);
    [temp1 temp2 temp3 field(z)]=textread(fid,'%s %s %s %f',1,'headerlines',9);
    [temp1 resistance(z)]=textread(fid,'%s %f',1,'headerlines',10);
    [temp1 resistance1]=textread(fid,'%s %f',1,'headerlines',10);
    [temp1 temp2 vbw]=textread(fid,'%s %s %f',1,'headerlines',11);
    [temp1 temp2 rbw]=textread(fid,'%s %s %f',1,'headerlines',12);
    [temp1 temp2 points]=textread(fid,'%s %s %f',1,'headerlines',13);
    [temp1 temp2 startf]=textread(fid,'%s %s %f',1,'headerlines',14);
    [temp1 temp2 stopf]=textread(fid,'%s %s %f',1,'headerlines',15);
...
%     WindowFreq = [min(frequency)/1e9 max(frequency)/1e9];
    ...
    if z == 1
        ...
        noise_dBW = mean(spectrum(noise_select_down(end):noise_select_up(1))) - mean(clean_spectrum(noise_select_down(end):noise_select_up(1))) - 30; % from dBm to dBW'
...
export_vector = [rff' r_count current rr];
for ii = 1 : GROUP
...
% Write text file
name=sprintf('%s.txt',strrep(strrep(strcat(Folder,fname, num2str(seqfig1_unique(c)), seqfig1units, '_', num2str(seqfig2_unique(m)), seqfig2units, '_', num2str(seqdev_unique(d)), seqdevunits, '_', sampleID), ' ', ''), '=', ''));
...
% figure; clf;
% % plot(abs(rff_log)*1e-6, peak_freq/1e9, '.');
% plot(abs(seqaxis_unique)*1e-6, peak_freq_org3/1e9, '.');
% 
% figure; clf;
% plot(abs(seqaxis_unique)*1e-6, int_power_org3/1e-12, '.');
% 
% figure; clf;
% plot(abs(seqaxis_unique)*1e-6, linewidth_org3/1e6, '.');
% 
% close

%% Save for outer loops
linewidth_out{d} = linewidth_org3;
int_power_out{d} = int_power_org3;
peak_power_out{d} = peak_power_org3;
peak_freq_out{d} = peak_freq_org3;
end
end
end