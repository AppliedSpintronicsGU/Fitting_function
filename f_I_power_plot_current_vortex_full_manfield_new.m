...
threshold = 2;           % Threshold for peak detection. NOTE this is not
                         ...
                         % of 2 is suggested.
N_attempt = 3;           % Allow several attempts to find the best fit. For
                         % low power peaks the default is 5, while for data
                         % with well defined peaks setting it to 1 is good
                         % enough.
gain = 33;               % Gain of the amplifier used during measurements
...
%     resistance = resistance - 6.7; % 6.7 Ohm is the measured lead
                                   ...
                                   % circulator.
%     refl_corr = ( (resistance(i) - R_biasTee - 50) / (resistance(i) - R_biasTee + 50) )^2;
...
figure; clf;
plot(abs(current_log)*1e3, peak_freq_org3/1e9, '.');
