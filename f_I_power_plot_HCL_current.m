...
N_attempt = 10;           % Allow several attempts to find the best fit. For
                         ...
                         % enough.
gain = 40;               % Gain of the amplifier used during measurements
R_biasTee = 6.7;         % Measured lead resistance including bias tee
dF = 50e6;              % Maximum frequency step in Hz for grouping. Default 50e6.
...
% Otherwise, runs the main program
    ...
    [current_log field_log angle_log spectrum_path]=textread(log_path,...
...
for i=1:length(current_log)
    ...
    [temp1 resistance(i)]=textread(fid,'%s %f',1,'headerlines',9);
    ...
    LowFreq = startf/1e9;             % Minimum frequency to start scan in GHz
...
%% Export variables
export_vector = [current field (resistance-R_biasTee)];
for ii = 1 : GROUP
