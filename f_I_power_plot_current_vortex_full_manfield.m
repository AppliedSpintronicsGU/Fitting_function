...
threshold = 1.6;           % Threshold for peak detection. NOTE this is not
...
amp_gain = 31;           % Gain of the amplifier used during measurements
...
REGROUP = 2;             % Set to 1 to sort based on frequency similarity
                         ...
                         % the "_org2" files.
LowFreq = 1e-3;             % Minimum frequency to start scan in GHz
WindowFreq = [1e-3 0.1];     % Window to estimate the measurement noise in GHz.
...
% Otherwise, runs the main program
    ...
    [current_log spectrum_path]=textread(log_path,...
        '%f %s',-1,'headerlines',7,'delimiter','\t'); % 7 is standard
end
...
% 	[temp1 temp2 field(i)]=textread(fid,'%s %s %f',1,'headerlines',6);
    [temp1 resistance(i)]=textread(fid,'%s %f',1,'headerlines',7);
    [temp1 resistance1]=textread(fid,'%s %f',1,'headerlines',7);
    [temp1 temp2 vbw]=textread(fid,'%s %s %f',1,'headerlines',8);
    [temp1 temp2 rbw]=textread(fid,'%s %s %f',1,'headerlines',9);
    [temp1 temp2 points]=textread(fid,'%s %s %f',1,'headerlines',10);
    [temp1 temp2 startf]=textread(fid,'%s %s %f',1,'headerlines',11);
    [temp1 temp2 stopf]=textread(fid,'%s %s %f',1,'headerlines',12);
...
%% Export variables
export_vector = [r_count current (resistance-R_biasTee)];
for ii = 1 : GROUP
...
figure; clf;
plot(abs(current)*1e3, peak_freq_org3/1e9, '.');

figure; clf;
plot(abs(current)*1e3, int_power_org3/1e-12, '.');

figure; clf;
plot(abs(current)*1e3, linewidth_org3/1e6, '.');