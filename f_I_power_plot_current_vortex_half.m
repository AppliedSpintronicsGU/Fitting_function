...
dF = 1e6;              % Maximum frequency step in Hz for grouping. Default 50e6.
REGROUP = 2;             % Set to 1 to sort based on frequency similarity
                         ...
                         % the "_org2" files.
LowFreq = 1e-1;             % Minimum frequency to start scan in GHz
WindowFreq = [1e-1 2];     % Window to estimate the measurement noise in GHz.
...
steps = length(unique(current_log));
...
for i=1:length(logindex)
    ...
    % Deletes the low frequencies of the spectrum
    if i == 1
...
%% Export variables
export_vector = [r_count field current (resistance-R_biasTee)];
for ii = 1 : GROUP
