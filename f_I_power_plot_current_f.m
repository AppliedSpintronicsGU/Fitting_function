...
%% Important Variables!
correction = 1;          % Allow power correction, 0 none, 1 low power,
                         % 2 high power (good for noisy data)
Max_peak = 12;           % Maximum peaks to fit
...
dF = 100e6;              % Maximum frequency step in Hz for grouping. Default 50e6.
...
for i=1:length(logindex)
    ...
    [temp1 temp2 startf]=textread(fid,'%s %s %f',1,'headerlines',12);
    [temp1 temp2 stopf]=textread(fid,'%s %s %f',1,'headerlines',13);

    LowFreq = startf;               % Minimum frequency to start scan in Hz
    WindowFreq = [startf (stopf-1e6)];    % Window to estimate the measurement noise in Hz.
    ...
    if i == 1
        low_point = find(frequency_pre <= LowFreq);
    end
    ...
    if i == 1
        ...
        noise_select_down = find(frequency <= WindowFreq(1));
        noise_select_up = find(frequency >= WindowFreq(2));
                ...
                elseif flag == 0
                    ...
                    for k = 1 : group_no
                        if isempty(output{i,k}) ~= 1
                            if abs(peak_freq(i,j) - peak_freq(output{i,k}(1),output{i,k}(2))) < 5e6
                                flag = 2;
                            end
                        end
                    end
...
%% Export variables
export_vector = [field current resistance];
for ii = 1 : GROUP
...
figure; clf;
plot(peak_freq_org2/1e9, '.');

figure; clf;
plot(int_power_org2/1e-12, '.');

figure; clf;
plot(linewidth_org2/1e6, '.');

% figure; clf;
% plot(abs(current)*1e3, peak_freq_org2/1e9, '.');
% 
% figure; clf;
